...
export default morgan((tokens, req, res) => {
  ...
  let length: any = tokens.res(req, res, 'content-length');
  let resTime: any = tokens['response-time'](req, res);
  let time = new Date(tokens.date(req, res) as string).toLocaleTimeString();
  let date = new Date(tokens.date(req, res) as string).toLocaleDateString('en-GB');
